# Open UltraCode Forkflow

## Branch Roles

- **origin/dev**: Clean mirror of OpenCode's dev branch.
- **origin/ultracode/dev**: Integration branch with Open UltraCode changes.
...
- **origin/ultracode/dev**: A stable integration point. **We build and release from this branch**.
...
2) Rebase integration branch on dev:
   - `git checkout ultracode/dev`
   - `git rebase dev`
   - `HUSKY=0 git push --force-with-lease origin ultracode/dev`

## Feature Work

1) Create a work branch:
   - `git checkout -b <name> ultracode/dev`
...
3) Rebase work branch on integration:
   - `git checkout <name>`
   - `git rebase ultracode/dev`
   - `HUSKY=0 git push --force-with-lease origin <name>`

4) Open PR: `<name>` → `ultracode/dev`.

5) Merge PR into `ultracode/dev`.
