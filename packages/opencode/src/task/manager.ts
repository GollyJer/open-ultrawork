...
// Lazy initialization - run orphan cleanup once on first start()
...
export namespace TaskManager {
  ...
  export async function start(input: StartInput): Promise<string> {
    // Lazy orphan cleanup - runs once on first task start
    if (!orphanCleanupDone) {
      orphanCleanupDone = true
      // Fire and forget - don't block task creation
      initOrphanCleanup().catch((err) => log.warn("Orphan cleanup failed", { error: err }))
    }
